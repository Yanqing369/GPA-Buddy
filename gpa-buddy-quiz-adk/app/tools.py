...
- 认证：Authorization: Bearer <JWT>（Google 登录后由客户端注入会话 state）
...
状态（token / turnstile / 当前测验）存在 ADK 会话的 state 里（tool_context.state），
本地桥（server.py）与云端 Agent Runtime 天然按会话隔离。
"""
...
def _headers(state) -> dict:
    return {"Authorization": f"Bearer {state.get('token')}"}


def _check_auth(state) -> dict | None:
...
def _list_cloud_banks(state) -> list[dict]:
    resp = httpx.get(f"{BASE}/api/cloud-banks", headers=_headers(state), timeout=TIMEOUT)
    resp.raise_for_status()
    return resp.json().get("banks", [])


def _find_mistake_bank(state, course_id: int | None) -> dict | None:
    banks = [b for b in _list_cloud_banks(state) if b.get("bank_type") == "mistake"]
...
def _read_bank_content(state, bank_id: int) -> dict:
    resp = httpx.post(f"{BASE}/api/share/{bank_id}/download",
                      headers=_headers(state), json={"password": ""}, timeout=TIMEOUT)
    resp.raise_for_status()
    return json.loads(resp.json()["content"])


def _get_or_create_mistake_bank(state, course_id: int | None) -> int:
    bank = _find_mistake_bank(state, course_id)
    if bank:
        return bank["id"]
    ...
    resp = httpx.post(f"{BASE}/api/cloud-banks", headers=_headers(state), json=body, timeout=TIMEOUT)
    resp.raise_for_status()
    return resp.json()["bank"]["id"]


def _append_mistakes(state, bank_id: int, wrong_questions: list[dict]) -> int:
    """把错题写进错题本；已存在的题累加 [N-times mistake] 计数。返回本次录入数。"""
    bank = _read_bank_content(state, bank_id)
    ...
    for q in wrong_questions:
        ...
        if text in existing:
            ...
            next_id += 1
        recorded += 1
    resp = httpx.put(f"{BASE}/api/cloud-banks/{bank_id}/content", headers=_headers(state),
                     json={"content": json.dumps(bank, ensure_ascii=False)}, timeout=TIMEOUT)
    resp.raise_for_status()
    return recorded


def _gen_questions_from_text(state, text: str, question_count: int, lang: str, course_id: int | None) -> list[dict]:
    """用文本出题：先 /text_generate（Vertex），失败则 /fallback/pdf_generate（DeepSeek）。"""
    turnstile = state.get("turnstile") or ""
...
def check_login(tool_context: ToolContext) -> dict:
    """检查当前登录状态，返回用户信息。任何操作前如果不确定是否已登录，先调用它。"""
    state = tool_context.state
    err = _check_auth(state)
    if err:
        return err
    resp = httpx.get(f"{BASE}/auth/me", headers=_headers(state), timeout=TIMEOUT)
...
def import_moodle_course(moodle_course_id: int, tool_context: ToolContext) -> dict:
    ...
    resp = httpx.post(f"{BASE}/api/moodle/courses/{moodle_course_id}/import",
                      headers=_headers(state), timeout=TIMEOUT)
    if resp.status_code >= 400:
...
def list_my_courses(tool_context: ToolContext) -> dict:
    """列出当前用户已拥有的课程（需要登录）。"""
    state = tool_context.state
    err = _check_auth(state)
    if err:
        return err
    resp = httpx.get(f"{BASE}/api/courses", headers=_headers(state), timeout=TIMEOUT)
    resp.raise_for_status()
...
def create_course(name: str, tool_context: ToolContext) -> dict:
    ...
    if err:
        return err
    resp = httpx.post(f"{BASE}/api/courses", headers=_headers(state), json={"name": name}, timeout=TIMEOUT)
    resp.raise_for_status()
    course = resp.json()["course"]
    return {"status": "success", "course_id": course["id"], "name": course["name"]}


def list_materials(course_id: int, tool_context: ToolContext) -> dict:
    """列出一门课程下的全部资料文件（需要登录）。
    ...
    if err:
        return err
    resp = httpx.get(f"{BASE}/api/courses/{course_id}", headers=_headers(state), timeout=TIMEOUT)
...
def upload_material(course_id: int, file_path: str, tool_context: ToolContext) -> dict:
    ...
    try:
        with open(file_path, "rb") as f:
            resp = httpx.post(f"{BASE}/api/courses/{course_id}/materials",
                              headers=_headers(state), files={"file": (name, f)},
...
def generate_quiz(course_id: int, material_ids: list[int], tool_context: ToolContext,
                  question_count: int = 20, lang: str = "zh") -> dict:
    ...
    body = {"materialIds": material_ids, "questionCount": question_count,
            "lang": lang, "turnstileToken": state.get("turnstile") or ""}
    try:
        with httpx.stream("POST", f"{BASE}/api/courses/{course_id}/generate-banks",
                          headers=_headers(state), json=body, timeout=TIMEOUT) as resp:
            if resp.status_code >= 400:
                resp.read()
...
def grade_quiz(answers: dict[str, str], tool_context: ToolContext) -> dict:
    ...
    if wrong:
        if state.get("token"):
            bank_id = _get_or_create_mistake_bank(state, quiz.get("course_id"))
            mistake_count = _append_mistakes(state, bank_id, wrong)
        else:
...
def list_mistakes(tool_context: ToolContext, course_id: int | None = None) -> dict:
    """查看错题本内容（需要登录）。
    ...
    content = _read_bank_content(state, bank["id"])
...
def generate_quiz_from_mistakes(tool_context: ToolContext, course_id: int | None = None,
                                question_count: int = 20, lang: str = "zh") -> dict:
    ...
    mistakes = _read_bank_content(state, bank["id"]).get("questions", [])
    if not mistakes:
        return {"status": "error", "message": "错题本是空的，无法根据错题出题。先做一次练习积累错题。"}
    ...
    try:
        questions = _gen_questions_from_text(state, "\n".join(lines), question_count, lang, course_id)
    except RuntimeError as e:
        return {"status": "error", "message": str(e)}
    if not questions:
        return {"status": "error", "message": "出题接口没有返回任何题目，请重试。"}
    state["quiz"] = {"course_id": course_id, "title": "错题重练", "questions": questions}
    return {"status": "success", "count": len(questions),
