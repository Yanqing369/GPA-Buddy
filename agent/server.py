...
from quiz_agent.state import session
...
async def set_turnstile(req: Request):
    ...
    body = await req.json()
    session["turnstile"] = (body.get("token") or "").strip() or None
    return {"ok": bool(session["turnstile"])}


@app.get("/api/status")
async def status():
    return {"logged_in": bool(session.get("token")), "turnstile": bool(session.get("turnstile"))}
